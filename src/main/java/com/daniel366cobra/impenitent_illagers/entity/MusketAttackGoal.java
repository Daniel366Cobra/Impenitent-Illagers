...
public class MusketAttackGoal<T extends InterloperIllagerEntity & MusketUser>
        extends Goal {
    ...
    private boolean isEntityHoldingMusket() {
        return this.actor.isHolding(ModItems.FLINTLOCK_MUSKET);
    }
        ...
        if (this.postHurtTicksLeft == 0) {
            switch (this.stage) {
                case UNLOADED: {
                    if (!targetNotLocked) {
                        //Loading weapon
                        this.actor.setCurrentHand(ProjectileUtil.getHandPossiblyHolding(this.actor, ModItems.FLINTLOCK_MUSKET));
                ...
                case READY_TO_ATTACK: {
                    if (canSeeTarget) {
                        ...
                        this.actor.setAiming(false);
                        ItemStack i = this.actor.getStackInHand(ProjectileUtil.getHandPossiblyHolding(this.actor, ModItems.FLINTLOCK_MUSKET));
